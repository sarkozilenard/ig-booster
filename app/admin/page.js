...
import { LogOut, RefreshCw, Plus, Copy, Trash2, ShieldCheck, ShoppingBag, Ticket, KeyRound, BarChart3, Users, TrendingUp, Clock } from "lucide-react";
...
export default function AdminDashboard() {
  ...
  const refresh = async () => {
    setRefreshing(true);
    try {
      const [s, o, c, cu] = await Promise.all([
        fetch("/api/admin/stats").then(r => r.json()),
        fetch("/api/admin/orders").then(r => r.json()),
        fetch("/api/admin/access-codes").then(r => r.json()),
        fetch("/api/admin/coupons").then(r => r.json()),
      ]);
      setStats(s); setOrders(o.orders || []); setCodes(c.codes || []); setCoupons(cu.coupons || []);
        ...
        <table className="w-full text-sm">
          <thead className="bg-purple-500/10">
            <tr className="text-left">
              <th className="p-3">Rendelés ID</th><th className="p-3">Név</th><th className="p-3">IG</th><th className="p-3">Mennyiség</th><th className="p-3">Összeg</th><th className="p-3">Státusz</th><th className="p-3">Idő</th><th className="p-3"></th>
            </tr>
          </thead>
          <tbody>
            ...
            {orders.map(o => (
              <tr key={o.orderId} className="border-t border-purple-500/10">
                ...
                <td className="p-3">@{o.instagramUsername}</td>
